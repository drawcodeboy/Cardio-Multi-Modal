import torch
import numpy as np
import os

def save_model_ckpt(model, model_name, current_epoch, save_weights_dir):
    ckpt = {}
    ckpt['model'] = model.state_dict()
    ckpt['epochs'] = current_epoch
    
    save_name = f"{model_name}_{current_epoch:03d}.pth"
    
    try:
        torch.save(ckpt, os.path.join(save_weights_dir, save_name))
        print(f"Save Model @epoch: {current_epoch}")
    except:
        print(f"Can\'t Save Model @epoch: {current_epoch}")
        
def save_loss_ckpt(model_name, current_epoch, train_loss, save_losses_dir):
    try:
        np.save(os.path.join(save_losses_dir, f'train_loss_{model_name}.npy'), np.array(train_loss))
        print('Save Train Loss')
    except:
        print('Can\'t Save Train Loss') 